...
متود ها شبیه به فانکشن ها هستن ولی متعلق به یک چیزین. برای یک چیز خاص یک کار خاصی رو انجام میدن.
جلوتر به صورت کاملا مفصل درباره اینا اشنا میشیم، ولی برای شروع همینکه بدونیم متود ها برای یک سری چیزا هستن کافیه. مثلا استرینگ ها یکسری متود ها دارن که برای استرینگ ها یکسری کارهارو انجام میدن. مثلا یکی از متود ها میاد استرینگ مارو تمام حروف هاشو بزرگ میکنه یا یکی کوچیک میکنه.
خلاصه هرکدوم از این استرینگ هارو بهر کاری ساخته اند !.


---

#### String Method : 
متود هایی که متلع به استرینگ ها در پایتون هستند رو string method میگن که مثالشو بالا براتون زدم. که چجوریاست.
برای اینکه ما از این متو ها استفاده بکنیم نیازه که بعد استرینگ یا بعد از متغیر حاوی یک استرینگ بیایم اون متود رو بهش اضافه بکنیم.
به عنوان مثال : 
```py
"hello".upper()
...
این متود کارش اینه که میاد حروف های استرینگ مارو به حروف های بزرگ تبدیل میکنه. 
مثلا اگه من در استرینگم،  یک حرف a داشته باشم اون میاد تبدیلش میکنه به A . یا اگه salam  داشته باشیم تبدیل میکنه به SALAM . به همین اسونی.
...
print(name) # خروجی : ALIREZA
```

الان میبینید که خروجی متغیر ما تغییر کرده.

#### .lower() : 

این متود دقیقا مثل متود بالایی هستش با این تفاوت جای اینکه بیاد حروف هارو بزرگ بکنه ، کوچیک میکنه. یعنی ما اگه داشته باشیم A این میکنه a.
به همین آسونی ! .
...
print(greet.find("be")) # خروجی : 3 . این عدد 3 همون ایندکس 3 هستش.

```

> نکته : ما میتونیم به متود فایند بگیم که از کجا تا کجای استرینگ مارو بگرده به این صورت که ایندکس اول اون کلمه ای که مدنظرمونه، ایندکس دوم اینکه از کجای استرینگ یا از کدوم ایندکس بگرده، و ایندکس سوم اینکه تا کجای استرینگ و تا کدوم ایندکس بگرده.

مثال : 
```py

greet = "to be or not to be"
print(greet.find("not", 8, 12)) # خروجی : 9
...
این متود هر حروفی، جمله ای ، کلمه ایکه ما بهش بگیم رو توی تمام اون جمله با چیزی که ما بهش میگیم عوض میکنه.
به عنوان مثال جمله to be or not to be رو در نظر بگیرید.
اگه من بهش بگم که be رو به duck عوض بکن ، این میاد تمامی be هارو با duck عوض میکنه.
و جمله ما میشه to duck or not to duck .

```py

greet = "to be or not to be"
greet = greet.replace("be", "duck")
print(greet) # خروجی : to duck or not to duck
...
بولین ها یک نوع داده در زبان برنامه نویسی پایتون هستند. این نوع داده دوتا هم بیشتر نیست.
بله !
خیر !
فقط همین ! . بله و خیر.
True به معنای بله هستش و False  به معنای خیر.
کابرد اینا کجاست ؟
به عنوان مثال فرض بکنید که شما یک وبسایتی دارید که قراره از کاربر یکسری اطلاعاتی رو بگیرید . یکی از اون اطلاعات ها این هستش که آیا ازدواج کرده یا خیر؟ خب این جواب دو گزینه بیشتر نداره شما یا باید بگید بله یا خیر.
همین ! .
...
> prons and cons : مزایا و معایب

#### تکرار استرینگ : 
شاید براتون جالب باشه . ما در زبان پایتون میتونیم استرینگ هارو بدون نیاز به حلقه ها که جلوتر دربارش یاد میگیرید تکرار بکنیم.
فقط کافیه اون استرینگ مورد نظر رو ضربدر اون تعداد بکنیم.
...
#### پروژه (مخفی کردن پسورد) : 
بریم باهم یک پروژه ای بسازیم که نام کاربری مارو ازمون میگیره و بعد پسورد مارو ازمون میگیره و نسبت به تعداد حروف پسوردمون اونو برای ما مخفی میکنه و جاش * میزاره.
...
خروجی : 
![hide password project](../src/img/hide_pass.png)
